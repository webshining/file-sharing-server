...
class AuthService {
	generateTokens = async (
		...
		const accessToken = jwt.sign(accessPayload, JWT_ACCESS_SECRET, { expiresIn: JWT_ACCESS_MINUTES * 60 });
		const refreshToken = jwt.sign(refreshPayload, JWT_REFRESH_SECRET, { expiresIn: JWT_REFRESH_MINUTES * 60 });
		await client.connect();
		await client.set(refreshToken, "refreshToken", { EX: JWT_REFRESH_MINUTES * 60 });
	...
	refreshDecode = async (token: string): Promise<{ err?: string; decode?: any }> => {
		...
		if (!exists) err = "Token does not exist";
		try {
